    ...
    f_b.add(system, "text")
      ...
      f_b.add(system, 'topSpeed', 0, 1000);
    ...
    class Particle {
      ...
      display() {
        ...
        fill(this.color);
        circle(this.position.x, this.position.y, this.size * s * map(this.velocity.mag(),0,system.topSpeed,0.5,2));
      }
      update() {
    ...
    function init() {
      ...
      for (let x = 0; x < width; x += step) {
        for (let y = 0; y < height; y += step) {
          let a = noise(x*0.01,y*0.01)*TWO_PI;
          field[`${x}-${y}`] = a;
          translate(x,y);
          rotate(a);
